...
Keep aligned with `../auto-repair-shop-api/.agents/policies/integration.md`.

This product pairs two repositories:

| Layer | Path | Role |
|-------|------|------|
| Django API | sibling `../auto-repair-shop-api` | Source of truth for persistence, auth, business rules, OpenAPI |
| Angular app | this repo (`auto-repair-shop-front`) | Sole first-party HTTP/WebSocket consumer of that API |
...
- Types: this app uses `npm run api:types` / `api:types:check` against `../auto-repair-shop-api/docs/openapi.yaml`; repositories remain manual and typed.
