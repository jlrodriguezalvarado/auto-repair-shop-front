# Continuous improvement (harness) — auto-repair-shop-front

Keep aligned with `../auto-repair-shop-api/.agents/policies/continuous-improvement.md` for the shared loop. Layer landing zones differ below.
...
| QA skipped or weak evidence | `policies/qa.md` (keep aligned with auto-repair-shop-api) |
| Cursor-session habit | `.cursor/rules/workflow.mdc` or a focused `.mdc` |

## Sibling and parent

- **Sibling-only API failure**: do not change this repo; note follow-up for `auto-repair-shop-api` in the plan.
- **Cross-layer failure**: apply the front-side defense here; require the matching API-side defense in `auto-repair-shop-api` (or accept an explicit “none — front-only root cause” in the plan).
