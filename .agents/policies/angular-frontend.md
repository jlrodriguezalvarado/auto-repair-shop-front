...
Keep aligned with sibling awareness in `../auto-repair-shop-api/.agents/policies/integration.md`.
...
- Data source: Django API in sibling `auto-repair-shop-api` via `environment.apiUrl` / `wsUrl`. Do not add parallel backends or invent undocumented endpoints.
...
- When generated OpenAPI types exist under `src/app/core/api/generated/`, regenerate them from the API OpenAPI artifact and never edit them manually.
...
- Prefer centralized UI strings when an i18n module exists (`src/app/core/i18n/`). Do not hardcode user-visible copy in templates/TS except temporary scaffolding.
- If key-generation scripts exist (`npm run i18n:keys`), keep generated key types in sync; do not hand-edit generated key unions.
...
# npm run api:types:check  # when the OpenAPI codegen pipeline exists
