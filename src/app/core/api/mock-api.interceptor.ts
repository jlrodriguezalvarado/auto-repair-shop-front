...
import { keysToSnake } from './case-mapper';
...
const MOCK_USERS: User[] = [
  { id: 0, username: 'superadmin', email: 'superadmin@saas.com', role: 'SUPER_ADMIN', isActive: true, company: null },
  {
    ...
    role: 'ADMIN',
    isActive: true,
    company: { id: 1, name: 'Taller Mecánico El Chasis Feliz, C.A.' },
  },
  {
    id: 2,
    username: 'sec',
    email: 'secretary@taller.com',
    role: 'SECRETARY',
    isActive: true,
    company: { id: 1, name: 'Taller Mecánico El Chasis Feliz, C.A.' },
  },
...
let mockCustomers: CustomerProfile[] = [
  {
    ...
    notes: 'Cliente frecuente, prefiere repuestos originales.',
  },
  {
    id: 2,
    firstName: 'María',
...
const mockServices: ServiceCatalog[] = [
  {
    ...
    description: 'Cambio de aceite de motor multigrado y filtro estándar.',
    basePrice: 45.0,
  ...
  },
  {
    ...
    description: 'Alineación de tren delantero y balanceo de 4 ruedas.',
    basePrice: 30.0,
  ...
  },
  {
    ...
    description: 'Inspección completa de pastillas, discos y fluido.',
    basePrice: 20.0,
  ...
  {
    id: 4,
    name: 'Entonación de Motor',
    description: 'Limpieza de inyectores, cambio de bujías y filtro de gasolina.',
    basePrice: 120.0,
    ...
    services: [
      {
        ...
        workOrder: 1,
        service: 2,
        nameSnapshot: 'Alineación y Balanceo',
        quantity: 1,
        unitPrice: 30,
        totalPrice: 30,
      },
      {
        ...
        workOrder: 1,
        service: 3,
        nameSnapshot: 'Revisión de Frenos',
        quantity: 1,
        unitPrice: 20,
        totalPrice: 20,
      },
    ...
    items: [
      {
        id: 1,
        workOrder: 1,
        name: 'Bujes de Meseta',
        quantity: 2,
        unitCost: 35,
        totalCost: 70,
        providedBy: 'workshop',
        supplierName: 'Repuestos El Pana',
        purchaseDate: '2026-01-05',
        notes: 'Garantía 6 meses.',
      },
      {
        id: 2,
        workOrder: 1,
    ...
    services: [
      {
        ...
        workOrder: 2,
        service: 1,
        nameSnapshot: 'Cambio de Aceite y Filtro',
        quantity: 1,
        unitPrice: 45,
        totalPrice: 45,
      },
...
export const mockApiInterceptor: HttpInterceptorFn = (
  ...
  if (companyHeader && isTenantEndpoint && !isSafeMethod) {
    return new Observable((observer) => observer.error({ status: 403, error: { detail: 'Read-only company view' } }));
  }
  ...
  if (endpoint.startsWith('/users/token/') && !endpoint.includes('refresh')) {
    ...
    responseBody = {
      ...
      recent_payments: mockReceipts.flatMap((r) =>
        (r.payments || []).map((p) => ({
          id: p.id,
          amount: p.amount,
          date: p.paymentDate,
          customer_name:
            mockCustomers.find((c) => c.id === r.customer)?.firstName +
            ' ' +
            mockCustomers.find((c) => c.id === r.customer)?.lastName,
        }))
      ),
      debtors: mockReceipts
    ...
    if (idMatch) {
      ...
      if (search) {
        ...
        list = list.filter(
          (c) =>
            c.name.toLowerCase().includes(s) || c.taxId.toLowerCase().includes(s) || c.email.toLowerCase().includes(s)
        );
      }
    ...
    if (idMatch) {
      ...
      } else if (endpoint.includes('/assign_mechanic/')) {
        const body = req.body as any;
        mockWorkOrders[index].assignedMechanic = body.mechanic_id ?? body.assigned_mechanic;
        responseBody = mockWorkOrders[index];
      ...
      if (endpoint.includes('/approve/')) {
        ...
        const newOrder: WorkOrder = {
          ...
          services: (est.services || []).map((s, idx) => ({
            ...
            service: 1,
            nameSnapshot: s.name ?? s.nameSnapshot ?? 'Service',
            quantity: s.quantity ?? 1,
            unitPrice: s.price ?? s.unitPrice ?? 0,
            totalPrice: s.total ?? s.totalPrice ?? 0,
          })),
          items: (est.items || []).map((it, idx) => ({
            ...
            unitCost: it.price ?? it.unitCost ?? 0,
            totalCost: it.total ?? it.totalCost ?? 0,
            providedBy: 'workshop' as const,
          })),
        };
        mockWorkOrders.push(newOrder);
        responseBody = newOrder;
    ...
    responseBody = method === 'POST' ? { id: 1, endpoint: (req.body as any)?.endpoint, is_active: true } : [];
